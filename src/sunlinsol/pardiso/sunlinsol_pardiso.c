...
SUNLinearSolver SUNLinSol_PARDISO(N_Vector y, SUNMatrix A) {
  SUNLinearSolver S;
  ...
  ops->getid = SUNLinSolGetID_PARDISO;
...
int SUNLinSol_PARDISOReInit(SUNLinearSolver S, SUNMatrix A, sunindextype nnz,
                     int reinit_type) {
  sunindextype n;
  int type;
...
// int SUNLinSol_PARDISOSetOrdering(SUNLinearSolver S, int ordering_choice)
//{
...
SUNLinearSolver_ID SUNLinSolGetID_PARDISO(SUNLinearSolver S)
{
  return(SUNLINEARSOLVER_PARDISO);
}
...
sunindextype SUNLinSolLastFlag_PARDISO(SUNLinearSolver S) {
