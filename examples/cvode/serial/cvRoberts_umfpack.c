/* -----------------------------------------------------------------
 * Programmer(s): Raymond Langer.
 *      Based on cvRoberts_dns.c and modified to use umfpack.
 * -----------------------------------------------------------------
 * Example problem:
 * 
 ...
 * Newton iteration with the Umfpack sparse direct linear solver, and a
...
#include <sunlinsol/sunlinsol_umfpack.h>    /* access to Umfpack sparse direct solver   */
...
int main()
{
  ...
  /* Create UMFPACK solver object for use by CVode */
  LS = SUNLinSol_UMFPACK(y, A);
  if(check_flag((void *)LS, "SUNLinSol_UMFPACK", 0)) return(1);
